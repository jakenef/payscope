    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
      <div style={{ position: 'relative', width: '196px', height: '196px' }}>
        <svg width="196" height="196" style={{ transform: 'rotate(-90deg)' }}>
          <circle cx="98" cy="98" r={radius} fill="none" stroke={track} strokeWidth="12" />
          <circle
        ...
        <div style={{
          ...
          <div style={{
            ...
            fontSize: '3.6rem', color, lineHeight: 1,
          }}>
            {score}
          </div>
          <div style={{
            fontFamily: 'var(--font-sans)', fontSize: '0.7rem', fontWeight: 500,
            color: 'var(--text-muted)', letterSpacing: '0.08em', marginTop: '4px',
          }}>
            / 100
          </div>
        </div>
...
export default function AuditScoreHero({ summary }) {
  const { biller_score, total_medicare_expected, total_paid, leakage_dollars } = summary
  const collectionPct = total_medicare_expected > 0
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{
            ...
            You billed{' '}
            <span style={{ color: 'var(--primary-light)' }}>{fmt(total_medicare_expected)}</span>
            {' '}and collected{' '}
            <span style={{ color: 'var(--green)' }}>{fmt(total_paid)}</span>
            {' '}— recovering{' '}
            <span style={{ color: 'var(--primary-light)' }}>{collectionPct}%</span>
            {' '}of what you billed.
          </div>
          <div style={{
            fontFamily: 'var(--font-serif)',
            ...
            You left{' '}
            <span style={{ color: 'var(--red)' }}>{fmt(leakage_dollars)}</span>
            {' '}on the table.
          </div>
        </div>
      </div>
